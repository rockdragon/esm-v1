...
func (c *Migrator) NewBulkWorker(docCount *int, pb *pb.ProgressBar, wg *sync.WaitGroup, fields Fields) {
	...
	for {
		...
		case docI, open := <-c.DocChan:
			...
			for k, v := range doc.source {
				if reflect.ValueOf(v).Kind() == reflect.Float64 && fields.TypeOf(k, "long") {
					doc.source[k] = int64(v.(float64))
					//log.Infof("key:%+v, value:%+v, value:%+v, relect:%+v, mapping: %+v",
					//	k,
					//	v,
					//	doc.source[k],
					//	reflect.ValueOf(doc.source[k]).Kind(),
					//	fields.GetType(k),
					//)
				}

				if reflect.ValueOf(v).Kind() == reflect.Slice && fields.TypeOf(k, "long") {
					temp_slice := v.([]interface{})
					dest_slice := new([]int64)
					for _, v2 := range temp_slice {
						*dest_slice = append(*dest_slice, int64(v2.(float64)))
					}
					doc.source[k] = *dest_slice
					//log.Infof("key:%+v, value:%+v, value:%+v, relect:%+v, mapping: %+v",
					//	k,
					//	v,
					//	doc.source[k],
